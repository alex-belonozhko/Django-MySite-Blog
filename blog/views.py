...
def post_detail(request, year, month, day, post):
    ...
    if request.method == 'POST':
        comment_form = CommentForm(data=request.POST) # We get already completed form fields from the request
...
def post_share(request, post_id):
    ...
    sent = False
    if request.method == 'POST': # the form was sent to save
        form = EmailPostForm(request.POST)
        if form.is_valid(): # all form fields are validated
